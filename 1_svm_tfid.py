...
def sparse_abs_diff(a, b):
    # différence absolue |a - b| pour matrices sparses
    diff = a - b
    diff = diff.tocsr()
...
def sparse_prod(a, b):
    # produit élément par élément a * b pour matrices sparses
    return a.multiply(b)
...
# Concaténation + différence absolue + produit élément par élément
# [v1 ; v2 ; |v1-v2| ; v1*v2] pour forcer le modèle à comparer les deux phrases
X_train_vec = hstack([
    X_train_s1_vec,
    X_train_s2_vec,
    sparse_abs_diff(X_train_s1_vec, X_train_s2_vec),
    sparse_prod(X_train_s1_vec, X_train_s2_vec),
])
...
X_dev_m_vec = hstack([
    X_dev_m_s1_vec,
    X_dev_m_s2_vec,
    sparse_abs_diff(X_dev_m_s1_vec, X_dev_m_s2_vec),
    sparse_prod(X_dev_m_s1_vec, X_dev_m_s2_vec),
])
...
X_dev_mm_vec = hstack([
    X_dev_mm_s1_vec,
    X_dev_mm_s2_vec,
    sparse_abs_diff(X_dev_mm_s1_vec, X_dev_mm_s2_vec),
    sparse_prod(X_dev_mm_s1_vec, X_dev_mm_s2_vec),
])
